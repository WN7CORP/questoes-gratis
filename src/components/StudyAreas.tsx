...
const StudyAreas = ({
  ...
  const fetchAreas = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        const areasArray: AreaCount[] = Object.entries(areaCounts).map(([area, count]) => ({
          area,
          count
    ...
    return <div className="h-full overflow-y-auto bg-netflix-black">
        <div className="p-6 px-[9px] py-[12px]">
          <div className="flex items-center gap-4 mb-6 p-4 rounded-lg bg-gray-800 border-l-4 border-netflix-red animate-fade-in">
            <button onClick={handleBackToAreas} className="text-netflix-red hover:text-red-400 transition-all duration-200 font-semibold hover:scale-105">
              ← Voltar às Áreas
            </button>
          ...
          <QuestionsSection selectedArea={selectedArea} onHideNavigation={onHideNavigation} />
        </div>
      </div>;
  }
  if (loading) {
  ...
  return <div className="h-full overflow-y-auto bg-netflix-black p-6">
      {/* Header */}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        return <Card key={area.area} className={`${colorScheme.bg} ${colorScheme.border} border-l-4 p-6 cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-xl group animate-fade-in`} style={{
          animationDelay: `${index * 50}ms`
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <Badge variant="outline" className={`${colorScheme.border} ${colorScheme.text} bg-black/20 transition-all duration-200 group-hover:scale-105`}>
                    {area.count} questões
                  </Badge>
                </div>
      ...
      {/* Stats Footer */}
      <div className="mt-12 p-6 bg-gradient-to-r from-netflix-red/10 to-red-800/10 border border-netflix-red/20 rounded-lg animate-fade-in" style={{
