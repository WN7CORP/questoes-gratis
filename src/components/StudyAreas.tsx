...
const StudyAreas = ({ onHideNavigation }: StudyAreasProps) => {
  ...
  const [filteredAreas, setFilteredAreas] = useState<{ area: string; count: number }[]>([]);
  ...
  const fetchAreas = async () => {
    try {
      ...
      if (error) {
        ...
        const areasWithCount = Object.entries(areaCount || {})
  ...
  const getAreaColors = (index: number) => {
    const colors = [
      ...
      'from-orange-900/30 to-orange-800/20 border-orange-700/50',
      'from-cyan-900/30 to-cyan-800/20 border-cyan-700/50',
    ];
      ...
      <div className="px-6 mb-6">
        <div className="relative max-w-md">
          ...
          <Input
            ...
            className="pl-10 bg-netflix-card border-netflix-border text-white placeholder-gray-400 focus:border-netflix-red"
          />
        </div>
      </div>
      ...
      <div className="pb-6 px-6">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredAreas.map((areaInfo, index) => (
              <Card 
                key={areaInfo.area} 
                className={`bg-gradient-to-br ${getAreaColors(index)} p-6 cursor-pointer hover:scale-[1.02] transition-all duration-300 group border-2`}
                onClick={() => handleAreaSelect(areaInfo.area)}
              >
