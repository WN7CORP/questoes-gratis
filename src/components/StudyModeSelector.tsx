            ...
            <div className="p-4 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  key,
                  label
                }) => <button key={key} onClick={() => handleModeSelect(key)} className={`w-full text-left p-4 rounded-xl text-base transition-all duration-200 active:scale-95 ${studyMode === key ? 'bg-netflix-red text-white shadow-lg' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
                      {label}
                    </button>)}
                </div>
              </div>
              ...
              <div>
                ...
                <select value={selectedAreaFilter} onChange={e => handleAreaSelect(e.target.value)} className="w-full bg-gray-800 border border-gray-600 rounded-xl px-4 py-4 text-white text-base focus:ring-2 focus:ring-netflix-red focus:border-netflix-red transition-all">
              ...
              <div className="space-y-3 pt-2">
                <Button onClick={handleShuffle} variant="outline" size="lg" className="w-full border-gray-600 text-gray-300 hover:bg-gray-800 bg-gray-800 py-4 text-base rounded-xl active:scale-95 transition-all duration-200">
                  <Shuffle size={20} className="mr-3" />
                  Embaralhar
                </Button>

                <Button onClick={handleReset} variant="outline" size="lg" className="w-full border-gray-600 text-gray-300 hover:bg-gray-800 bg-gray-800 py-4 text-base rounded-xl active:scale-95 transition-all duration-200">
            ...
            <div className="fixed top-20 right-4 z-50 w-80 max-w-[90vw]">
              <Card className="bg-netflix-card border-netflix-border p-4 shadow-xl animate-scale-in">
                ...
                <div className="mb-4">
                  ...
                  key,
                  label
                }) => <button key={key} onClick={() => setStudyMode(key)} className={`w-full text-left p-2 rounded text-sm transition-colors ${studyMode === key ? 'bg-netflix-red text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
                        {label}
                      </button>)}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <select value={selectedAreaFilter} onChange={e => setSelectedAreaFilter(e.target.value)} className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                    <option value="">Todas as áreas</option>
                    {areas.map(area => <option key={area} value={area}>{area}</option>)}
                  </select>
                </div>
                ...
                onShuffle();
                setIsOpen(false);
              }} variant="outline" size="sm" className="w-full border-gray-600 text-gray-300 hover:bg-gray-800 bg-gray-800">
