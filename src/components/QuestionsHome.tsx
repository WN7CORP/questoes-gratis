...
const QuestionsHome = ({
  ...
  const fetchQuestionStats = async () => {
    try {
      ...
      const {
        count: total
      } = await supabase.from('QUESTOES_FINAL').select('*', {
        count: 'exact',
        head: true
      });
      ...
      const {
        data: temasData
      } = await supabase.from('QUESTOES_FINAL').select('tema').not('tema', 'is', null);
      const uniqueTemas = temasData ? new Set(temasData.map(item => item.tema)).size : 0;

      // Count distinct subjects
      const {
        data: assuntosData
      } = await supabase.from('QUESTOES_FINAL').select('assunto').not('assunto', 'is', null);
  ...
  if (showStudySession) {
    return <StudySessionFinal filters={selectedFilters} onExit={handleExitStudySession} />;
  }
  return <div className="bg-netflix-black p-4 pb-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center px-4">
          ...
          {totalQuestions === 0 && <p className="text-gray-400 text-xs sm:text-sm mt-2">
