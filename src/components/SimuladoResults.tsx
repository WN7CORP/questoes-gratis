...
import { Trophy, Target, Clock, TrendingUp, TrendingDown, Award, GraduationCap, CheckCircle, XCircle, ArrowLeft } from 'lucide-react';
interface AreaStats {
  area: string;
  correct: number;
  total: number;
...
const SimuladoResults = ({
  ...
  const improvement = previousAttempt ? sessionStats.correct - previousAttempt.correct_answers : 0;
  ...
  return <div className="min-h-screen bg-netflix-black p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card className="bg-netflix-card border-netflix-border p-6 text-center">
          <div className="mb-4">
            {passed ? <Trophy className="text-yellow-500 mx-auto mb-2" size={64} /> : <Target className="text-blue-500 mx-auto mb-2" size={64} />}
          </div>
          ...
          <div className="text-6xl font-bold mb-4">
            {passed ? <span className="text-green-500">{percentage}%</span> : <span className="text-orange-500">{percentage}%</span>}
          </div>
          
          <Badge variant="outline" className={`text-lg px-4 py-2 mb-4 ${passed ? 'border-green-500 text-green-300 bg-green-900/20' : 'border-orange-500 text-orange-300 bg-orange-900/20'}`}>
            {passed ? <>
          ...
          {!passed && <p className="text-gray-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-netflix-card border-netflix-border p-4 text-center">
            {improvement >= 0 ? <TrendingUp className="text-green-400 mx-auto mb-2" size={32} /> : <TrendingDown className="text-red-400 mx-auto mb-2" size={32} />}
        ...
        {/* Performance by Area */}
        {areaStats.length > 0 && <Card className="bg-netflix-card border-netflix-border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {bestArea && <Card className="bg-green-900/20 border-green-600 p-4">
              ...
              {worstArea && worstArea !== bestArea && <Card className="bg-orange-900/20 border-orange-600 p-4">
                  <div className="flex items-center justify-between">
                    <div>
            ...
            <div className="space-y-3">
              {areaStats.map((area, index) => <div key={area.area} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className={`text-lg font-bold ${area.percentage >= 70 ? 'text-green-400' : area.percentage >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                      {area.percentage}%
                    </div>
                  </div>
                </div>)}
        ...
        {/* Previous Attempts */}
        {previousAttempts.length > 0 && <Card className="bg-netflix-card border-netflix-border p-6">
            <h3 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
            ...
            return <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                    <div>
                    ...
                    <div className="text-right">
                      <div className={`text-lg font-bold ${attemptPercentage >= 70 ? 'text-green-400' : attemptPercentage >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                        {attemptPercentage}%
                      </div>
                      <div className="text-gray-400 text-sm">
        ...
        <div className="flex gap-4 justify-center">
          <Button onClick={onClose} className="bg-netflix-red hover:bg-red-700 text-white px-8 py-3">
